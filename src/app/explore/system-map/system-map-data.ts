...
export type EngineeringRelevance =
  | "primary_engineering_reference"
  | "supporting_assurance_reference"
  | "regulatory_architecture"
  | "not_an_engineering_instrument";

export type TreatyRelationship =
  | "1958_agreement_architecture"
  | "1998_agreement_architecture"
  | "jurisdiction_specific_participation"
  | "not_applicable";

export type DomesticImplementation =
  | "directly_applicable_only_where_adopted"
  | "requires_domestic_or_regional_implementation"
  | "may_be_referenced_by_applicable_regime"
  | "not_applicable";

export type LegalApplicability =
  | "voluntary_unless_incorporated"
  | "instrument_and_jurisdiction_specific"
  | "framework_not_standalone_requirement"
  | "concept_only"
  | "unverified";

export type AtlasResearchStatus =
  | "verified_core"
  | "jurisdiction_research_required"
  | "unverified_scaffold";

export type SystemMapDimensions = {
  engineeringRelevance: EngineeringRelevance;
  treatyRelationship: TreatyRelationship;
  domesticImplementation: DomesticImplementation;
  legalApplicability: LegalApplicability;
  atlasResearchStatus: AtlasResearchStatus;
};
...
export type SystemNode = {
  ...
  source?: string;
  sourceProvenance?: {
    access: "public_abstract" | "licensed_full_text" | "official_legal_reference";
    treatment: "source_summary" | "atlas_explanatory_shorthand";
  };
  learning?: LearningNote;
  learningConceptId?: LearningConceptId;
};

export function getSystemMapDimensions(node: SystemNode): SystemMapDimensions {
  if (node.jurisdiction) {
    return {
      engineeringRelevance: "not_an_engineering_instrument",
      treatyRelationship: "not_applicable",
      domesticImplementation: "not_applicable",
      legalApplicability: "unverified",
      atlasResearchStatus: "unverified_scaffold",
    };
  }

  if (node.id === "agreement-1958" || node.id === "un-regulations") {
    return {
      engineeringRelevance: "regulatory_architecture",
      treatyRelationship: "1958_agreement_architecture",
      domesticImplementation: "directly_applicable_only_where_adopted",
      legalApplicability: "instrument_and_jurisdiction_specific",
      atlasResearchStatus: "jurisdiction_research_required",
    };
  }

  if (node.id === "agreement-1998" || node.id === "un-gtrs") {
    return {
      engineeringRelevance: "regulatory_architecture",
      treatyRelationship: "1998_agreement_architecture",
      domesticImplementation: "requires_domestic_or_regional_implementation",
      legalApplicability: "instrument_and_jurisdiction_specific",
      atlasResearchStatus: "jurisdiction_research_required",
    };
  }
  ...
  if (node.nodeType === "regulation") {
    return {
      engineeringRelevance: "regulatory_architecture",
      treatyRelationship: "jurisdiction_specific_participation",
      domesticImplementation: "directly_applicable_only_where_adopted",
      legalApplicability: "instrument_and_jurisdiction_specific",
      atlasResearchStatus: "jurisdiction_research_required",
    };
  ...
  if (node.nodeType === "standard") {
    return {
      engineeringRelevance: "primary_engineering_reference",
      treatyRelationship: "not_applicable",
      domesticImplementation: "may_be_referenced_by_applicable_regime",
      legalApplicability: "voluntary_unless_incorporated",
      atlasResearchStatus: "jurisdiction_research_required",
    };
  }

  if (node.nodeType === "concept" || node.nodeType === "methodology") {
    return {
      engineeringRelevance: "supporting_assurance_reference",
      treatyRelationship: "not_applicable",
      domesticImplementation: "not_applicable",
      legalApplicability: "concept_only",
      atlasResearchStatus: "verified_core",
    };
  }
  ...
  return {
    ...
    treatyRelationship: "jurisdiction_specific_participation",
    domesticImplementation: "not_applicable",
    legalApplicability: "framework_not_standalone_requirement",
    atlasResearchStatus: "verified_core",
  };
}
...
export const JURISDICTIONS: {
  ...
  status: "verified" | "research_pending" | "expert_review_required" | "unverified_scaffold";
  sourceHints?: string[];
  sourceNote?: string;
}[] = [
  { key: "eu", label: "EU", level: "supranational", status: "research_pending" },
  { key: "nl", label: "Netherlands", level: "national", parent: "eu", status: "verified" },
  { key: "de", label: "Germany", level: "national", parent: "eu", status: "verified" },
  { key: "fr", label: "France", level: "national", parent: "eu", status: "unverified_scaffold" },
  { key: "es", label: "Spain", level: "national", parent: "eu", status: "unverified_scaffold" },
  { key: "it", label: "Italy", level: "national", parent: "eu", status: "unverified_scaffold" },
  { key: "at", label: "Austria", level: "national", parent: "eu", status: "unverified_scaffold" },
  {
    key: "uk",
  ...
  { key: "us", label: "United States", level: "national", status: "verified" },
  { key: "ca", label: "California", level: "subnational", parent: "us", status: "verified" },
  { key: "cn", label: "China", level: "national", status: "unverified_scaffold" },
];
...
export const NODES: SystemNode[] = [
  ...
  },
  {
    ...
    sourceProvenance: {
      ...
      treatment: "atlas_explanatory_shorthand",
    },
  ...
  },
  {
    ...
    nodeType: "standard",
    issuingBody: "ISO — International Organization for Standardization",
    whatItIs: "Technical standard addressing unreasonable risk arising from functional insufficiencies of the intended functionality, including reasonably foreseeable misuse within scope",
    geography: "International",
    legalEffect: "Voluntary ISO standard",
    takeaway: "SOTIF addresses unreasonable risk arising from functional insufficiencies where a relevant ISO 26262 malfunction is not necessary.",
  ...
  },
  {
    ...
    whatItIs: "The UNECE New Assessment/Test Method for ADS validation, combining five evidence pillars: audit/assessment, simulation or virtual testing, track testing, real-world testing, and in-service monitoring/reporting",
    ...
    learning: {
      plain: "NATM is UNECE's multi-pillar approach to assessing automated driving systems through audit/assessment, simulation, track tests, real-world tests and in-service monitoring/reporting. A scenario catalogue supports those pillars; it is not a sixth pillar.",
